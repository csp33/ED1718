...
template <class T>
void VectorDinamico<T>::Swap(const T a,const T b){
  T temp;
  temp=vector[a];
  vector[a]=vector[b];
  vector[b]=temp;
...
template <class T>
int VectorDinamico<T>::Buscar (const T &elemento) const {
  int posicion=-1;
  for(int i=0;i<GetTamanio() && posicion == -1 ; i++)
...
void VectorDinamico<T>::operator--(){
  tam--;
  Redimensiona(tam);
}
